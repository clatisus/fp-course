...
instance Functor ListZipper where
  (<$>) k (ListZipper l f r) = ListZipper (k <$> l) (k f) (k <$> r)
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus k (ListZipper l f r) = ListZipper l (k f) r
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper l f r) =
  case break p l of
    (_, Nil) -> isNotZ
    (a, b :. bs) -> isZ (ListZipper bs b (reverse a ++ f :. r))
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper l f r) =
  case break p r of
    (_, Nil) -> isNotZ
    (a, b :. bs) -> isZ (ListZipper (reverse a ++ f :. l) b bs)
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil f r) = ListZipper b a Nil
  where (a, b) = foldLeft (\(x, y) z -> (z, x :. y)) (f, Nil) r
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l f Nil) = ListZipper Nil a b
  where (a, b) = foldLeft (\(x, y) z -> (z, x :. y)) (f, Nil) l
...
moveLeftN n a = moveLeftN (n - 1) -<< moveLeft a
...
moveRightN n a = moveRightN (n - 1) -<< moveRight a
...
moveLeftN' n a =
  let moveLeftN'' n' a' q
        ...
        | otherwise =
          case moveLeft a' of
            MLZ Empty -> Left q
            MLZ (Full a'') -> moveLeftN'' (n' - 1) a'' (q + 1)
  in moveLeftN'' n a 0
...
moveRightN' ::
  ...
  -> Either Int (ListZipper a)
moveRightN' n a =
  let moveRightN'' n' a' q
        | n' == 0   = Right a'
        | n' <  0   = moveLeftN' (-n') a'
        | otherwise =
          case moveRight a' of
            MLZ Empty -> Left q
            MLZ (Full a'') -> moveRightN'' (n' - 1) a'' (q + 1)
  in moveRightN'' n a 0
...
--
-- the right side of <*> is of type:
--   (List x -> List x) . (ListZipper x -> List x) . (x -> ListZipper x)
-- which is: (x -> List x)
--
-- the left side of <*> is of type:
--   ((x -> Bool) -> List x -> Bool) . (x -> x -> Bool)
-- which is: (x -> List x -> Bool)
--
-- so after <*> is applied, the result type is: x -> Bool
--
...
-- /Tip:/ Use `zipWith`
  ...
  (<*>) (ListZipper fl ff fr) (ListZipper l f r) = ListZipper (zipWith ($) fl l) (ff f) (zipWith ($) fr r)
...
instance Extend ListZipper where
  ...
  (<<=) f z =
    ListZipper
      (unfoldr ((g <$>) . toOptional . moveLeft) z)
      (f z)
      (unfoldr ((g <$>) . toOptional . moveRight) z)
    where g = \z' -> (f z', z')
...
instance Traversable ListZipper where
  ...
  traverse k (ListZipper l f r) = ListZipper . reverse <$> traverse k (reverse l) <*> k f <*> traverse k r
...
instance Traversable MaybeListZipper where
  ...
  traverse k (MLZ Empty) = pure isNotZ
  traverse k (MLZ (Full a)) = isZ <$> traverse k a
