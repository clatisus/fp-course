...
(|||) p1 p2 = P $ \i ->
  let r1 = parse p1 i
  in if isErrorResult r1 then parse p2 i else r1
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) f p = P $ \i -> onResult (parse p i) (\r a -> parse (f a) r)
...
phoneParser ::
  Parser Chars
phoneParser = digit .:. phoneBodyParser >>= \p -> is '#' >> pure p
