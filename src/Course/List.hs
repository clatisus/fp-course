...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight (M.liftM2 (:.)) (M.return Nil)
...
find ::
  ...
  -> List a
  -> Optional a
find f as = headOr Empty (map A.pure $ filter f as)
...
-- Impossible!
-- see: https://github.com/tonymorris/fp-course/blob/master/src/Course/List.hs#L340
